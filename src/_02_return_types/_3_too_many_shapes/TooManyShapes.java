...
public class TooManyShapes {
	...
	public static void main(String[] args) {
		...
		String shapes = JOptionPane.showInputDialog("How many shapes do you want to have? ");
		int shape = Integer.parseInt(shapes);
		//3. Call canMakeShape() and save what is returned into a variable
		boolean selection = canMakeShape(side);
		//4. If the shape CAN be drawn
		int degrees = 360/side;
		if (selection == true) {
			calculateTurnAngle(side);
			drawPolygons(side, shape, degrees);
		}else {
			notEnoughSides();
